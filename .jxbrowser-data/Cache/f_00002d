...
;(function(window, document) {
  ...
  if (window.location.hostname == 'software-dl.ti.com')
  {
    // Use CDNJS to get the fonts online, so that it works.
    var fontStyleText = "\
        @font-face {\
  ...
  else
  {
    var fontStyleText = "\
        @font-face {\
          font-family: 'Glyphicons Halflings';\
          src: url('"+originBase+"/fonts/glyphicons-halflings-regular.eot');\
          src: url('"+originBase+"/fonts/glyphicons-halflings-regular.eot?#iefix') format('embedded-opentype'), url('"+originBase+"/fonts/glyphicons-halflings-regular.woff2') format('woff2'), url('"+originBase+"/fonts/glyphicons-halflings-regular.woff') format('woff'), url('"+originBase+"/fonts/glyphicons-halflings-regular.ttf') format('truetype'), url('"+originBase+"/fonts/glyphicons-halflings-regular.svg#glyphicons_halflingsregular') format('svg');\
        }\
    ";
  }

  // Add a try block and some maneuvering because some people use IE7 or compatibility mode
  try {
    var overrideFontStyleLink = document.createElement('style');
    overrideFontStyleLink.type = 'text/css';
    if (overrideFontStyleLink.styleSheet){
      overrideFontStyleLink.styleSheet.cssText = fontStyleText;
    } else {
      overrideFontStyleLink.appendChild(document.createTextNode(fontStyleText));
    }

    document.head.appendChild(overrideFontStyleLink);
  }
  catch(err)
  {/* Ignore and just render badly */}
