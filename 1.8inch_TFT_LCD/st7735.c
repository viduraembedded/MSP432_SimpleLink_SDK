...
extern SPI_Transaction lcdTransaction;
extern SPI_Handle lcdHandle;

uint8_t r=0;

#define TFT_CS_H()	GPIO_write(CONFIG_GPIO_CS_PIN, 1)
#define TFT_CS_L()	GPIO_write(CONFIG_GPIO_CS_PIN, 0)
#define TFT_DC_D()	GPIO_write(CONFIG_GPIO_DC_PIN, 1)
#define TFT_DC_C()	GPIO_write(CONFIG_GPIO_DC_PIN, 0)
#define TFT_RES_H()	GPIO_write(CONFIG_GPIO_RST_PIN, 1)
#define TFT_RES_L()	GPIO_write(CONFIG_GPIO_RST_PIN, 0)
...
static void ST7735_WriteCommand(uint8_t cmd)
{
    ...
    lcdTransaction.count = sizeof(cmd);
    lcdTransaction.txBuf = &cmd;
    spiTranferOK = SPI_transfer(lcdHandle, &lcdTransaction);

    if (spiTranferOK == false) {
...
static void ST7735_WriteData(uint8_t* buff, size_t buff_size)
{
    ...
    lcdTransaction.count = buff_size;
    lcdTransaction.txBuf = buff;
    spiTranferOK = SPI_transfer(lcdHandle, &lcdTransaction);
...
void ST7735_FillRectangle(uint16_t x, uint16_t y, uint16_t w, uint16_t h, uint16_t color)
{
    ...
    for(y = h; y > 0; y--)
    {
        for(x = w; x > 0; x--)
        {
            ...
            lcdTransaction.count = sizeof(data);
            lcdTransaction.txBuf = data;
            spiTranferOK = SPI_transfer(lcdHandle, &lcdTransaction);
            ...
            //spi_write_blocking(ST7735_SPI_PORT, data, sizeof(data));
#endif
        }
    }
    TFT_CS_H();
...
/*Demo function based on st7735 library*/
...
void DemoTFT(void)
{
    ...
    for (i = 0; i < ST7735_GetHeight(); i += 4)
    {
        ST7735_DrawFastHLine(0, i, ST7735_GetWidth() - 1, ST7735_WHITE);
    }

    for (i = 0; i < ST7735_GetWidth(); i += 4)
    {
    ...
    // Draw circles
    ST7735_FillScreen(ST7735_BLACK);
    for (i = 0; i < ST7735_GetHeight() / 2; i += 2)
    {
